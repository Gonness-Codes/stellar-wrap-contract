...
In Soroban (Stellar's smart contract platform), the execution and storage model differs significantly from EVM:1
...
This native capability is already fully implemented in `src/admin.rc` (`upgrade` function). It allows the contract logic (WASM) to be updated in-place while retaining the identical Contract ID and all existing persistent/instance storage.
